...
Codeshark separates **task tiers** from **execution roles**. Triage selects one task tier—Quick, Routine, Standard, Deep, or High assurance—before administrator work begins in a read-only, network-disabled, ephemeral session. It receives the request, active project, and relevant project memory/assets, but has no MCP or persistent session. Task tiers decide which roles run: Direct execution serves both Quick and Routine; Standard adds independent review; Deep adds Planning and one rework/recheck loop; High assurance also adds Research and two loops. Triage, Planning, Research, Primary execution, Independent review, Adversarial review, Rework, and Finalization are reusable roles with separately configurable model and reasoning settings. If Triage fails to return a valid structured decision, Codeshark safely falls back to Standard. The dedicated profile disables Fast mode.
